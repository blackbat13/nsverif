...
(*	This refers to the random seed of the program.	*)
open Sys;;
let value = 0;;

(*	This refers to the duration allowed for Tamarin to finish 
	analysing a program. It is of type float.
*)
let duration_try = 240.;;
...
(*	This function gathers all restrictions that will be used to 
	...
	Also note that l0 refers to restrictions that contains the fact 
	'Learn', a fact that is only present in the Selene voting protocol. 
	If Tamarin is launched on a file that does not contain the fact 
	'Learn' with a restriction that refers to it, then it shall 
	indicate that a 'wellformedness check' failed. However, Tamarin 
	will still work as if that restriction did not appear at all.
*)
...
*)
  in  
  let t0 = Array.of_list l1
  and t1 = Array.of_list l2
  and t2 = Array.of_list l3
  and t3 = Array.of_list l4
  in
  (t0,t1,t2,t3)
;;
